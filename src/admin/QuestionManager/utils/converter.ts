...
export async function loadYamlQuestions(yamlContent: string, metadata: QuestionMetadata): Promise<Question[]> {
  try {
    ...
    // Validate difficulty levels before conversion
    const validQuestions = rawQuestions.map(q => ({
      ...q,
      difficulty: validateDifficultyLevel(q.difficulty)
    }));
    
    return validQuestions.map(q => convertRawToQuestion(q, metadata));
  } catch (error) {
...
/**
 * Validates and converts a number to DifficultyLevel
 */
function validateDifficultyLevel(level: number): DifficultyLevel {
  if (level >= 1 && level <= 5) {
    return level as DifficultyLevel;
  }
  throw new Error(`Invalid difficulty level: ${level}. Must be between 1 and 5.`);
} 